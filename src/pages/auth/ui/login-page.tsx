...
export const LoginPage = () => {
	...
	const handleLogin = platform === 'max' ? handleMaxLogin : handleSsoLogin;
	const isLoading = isWeb ? isSsoLoading : isTgLoading;
	const error = isWeb ? null : maxError;
				...
				<View style={[styles.card, isWeb && styles.webCard]}>
					...
					{platform !== 'tg' && (
						<TouchableOpacity
							...
							onPress={() => Linking.openURL(EndPoints.bot_max)}
							disabled={isTgLoading || isSsoLoading}>
							{isTgLoading ? (
								<ActivityIndicator color={Colors.white} />
							) : (
								<Text style={styles.buttonText}>Открыть в Max</Text>
							)}
						</TouchableOpacity>
					)}
